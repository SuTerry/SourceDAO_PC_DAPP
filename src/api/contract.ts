  ...
  constructor() {
    (async () => {
      ...
      if (provider) {
        ...
        this._writeContract = new ethers.Contract(this.address, this.writeAbi, signer)

        this._readContract = new ethers.Contract(this.address, this.readAbi, provider)
      }

    })()
  }

  protected writeContract(api: string, ...arg: unknown[]): Promise<ContractResult> {
